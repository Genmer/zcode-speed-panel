...
- 三形态：**桌宠**（默认 200×200，默认宠物鲸鱼女仆 maid-deepseek-whale）/ **迷你仪表**（128×128 正方形）/ **速度胶囊**（172×72）。完整面板右上角**自绘下拉**切换（深色弹层，点击选项/外部、Esc、窗口失焦均关闭；原生 `<select>` 因 WebView2 弹层跟随系统浅色主题不可读而弃用，见 key-rules #8），选择持久化。
