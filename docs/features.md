...
- **原生全屏中收起的竞态（2026-09-19）**：全屏状态下收起悬浮窗（红点/Cmd+Q/托盘），须先 `set_fullscreen(false)` 退出全屏，**等约 700ms 让系统还原动画走完再套悬浮窗尺寸**——过渡期 `set_size` 会被系统还原帧覆盖，出现"全屏大小的悬浮窗"（用户实测报告）。`switch_mode` 已内置该守卫。
