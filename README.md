...
- **网络流量与上传监控**（完整面板"网络流量与上传监控"卡，左右双栏）：实时显示**整机上传/下载速度**与当日总量（接口计数器实测，**整机 = 本机全部应用流量而非仅 ZCode**，卡上有显式标注；约 1 秒滑窗、字节单位——任务管理器显示比特且仅统计所选网卡，读数对不上是口径差异，悬停数值可看完整说明）；并把上传拆分为**会话流量（≈ 估算，按 token×系数）**与**非会话上传（快照工件，真实值）**——后者监控 `~/.zcode/v2/checkpoints/` 的上传接受事件，ZCode 把整个工作区打包加密上传到云端时，这里会如实累计工件大小（**今日 N 个工件，下方逐行列出工作区名单，悬停看逐条明细**）并在上传进行中脉冲提示，卡片右栏的**快照上传记录列表**逐工作区列出已上传快照（时间/工作区/加密后大小/状态——上传中、待传、已接受；固定显示 7 行、其余列表内滚动查看，**可选中复制或一键导出为文本文件**留存取证）；另有 **ZCode 连接归属**（悬停看每条连接的远端与**归属进程**）：**ZCode 会话进程**（CLI，对话流量）与 **ZCode 桌面端**（ZCode 的 Electron 壳——快照上传等非对话流量的承载者）各持有几条外连——两组都是 ZCode 自身进程，不含其他应用。整机上传飙升 + 桌面端新连接 + "快照上传进行中"同时出现，就是又一次快照上传的现场证据（口径细节见 `docs/features.md`；按进程直测网络字节在 Windows/macOS 非管理员下无公开原语，故采用此分层设计）。当日网络累计跨重启续算；checkpoints 目录被 ACL 封锁时如实显示"不可读"。
