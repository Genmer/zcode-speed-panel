...
- **快照防护（阻断工作区快照静默上传，仅 macOS）**：ZCode 登录后会把**整个工作区（含 `.git` 历史）**加密上传到云端，设置开关无效（[机制分析文章](https://blog.ferstar.org/posts/zcode-silent-workspace-snapshot-upload/)）。完整面板"快照防护"卡一键开启：先把当时的上传记录**留档备份**（清单可回看），再清空本地已积累的加密快照并对 `~/.zcode/v2/checkpoints/` 加**文件系统不可变锁（chflags uchg）**——ZCode 写不进去，快照上传链路彻底死亡；**不碰网络，模型对话 / 代码补全 / 工具调用完全不受影响**，唯一损失是 ZCode 的「检查点回滚 / 时间线」功能（开启前确认弹窗逐条明示，随时可解除、目录自动重建）。防护中卡片累计"开启后 N 轮对话 · 新快照落盘 0 个"，上传记录区完整保留防护前的历史清单；卡片标题悬停可看防护原理与监控边界说明。
