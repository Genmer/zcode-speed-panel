...
完整面板：当前速度 / 今日平均 / 今日总量三块仪表 + 近 15 分钟速度曲线 + 状态栏（当前速度弧线随速度分档变色：0–30 绿 / 30–60 黄 / 60+ 红；当前速度卡右上角另有"上轮"小圆环，显示最近一次已完成调用的速度）
...
- **悬浮窗模式**（点右上角高亮的"⧉ 收起为悬浮窗"，或直接点窗口关闭按钮）
  - 三种样式可选（完整面板右上角下拉框）：**桌宠**（默认鲸鱼女仆，200×200；**滚轮上下滚动缩放** 100~480；双击或 🔄 换宠物；生成中跑步、待机站立；**待机时不显示气泡**，生成/估算时头顶气泡显示实时速度，**鼠标悬停变两行：实时速度 / 上轮均速**，此时宠物会略微缩小给气泡让位、不会被挡住）/ **迷你仪表盘**（140×116，主环旁带"上轮"小环）/ **速度胶囊**（224×78，实时速度下方一行"上轮均速"）
...
- **速度分档配色**：当前速度表的弧线与读数随速度整体变色——0–30 t/s 绿、30–60 t/s 黄、60+ t/s 红，一瞥即知快慢；背景轨道恒为灰色，悬浮窗迷你仪表与"上轮"小环同规则
- **"上轮"小表**（完整面板当前速度卡右上角、迷你仪表悬浮窗主环旁）：显示**最近一次已完成调用**的速度（落盘口径：输出+思考 ÷ 纯生成时长），与实时读数互为参照——实时测速受管道静默影响时，可对照上一轮实际速率判断当前读数是否可信；胶囊悬浮窗与桌宠悬停气泡同样给出该值；均按 0–30/30–60/60+ 分档变色，鼠标悬停有口径说明
