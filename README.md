...
5. **扣心跳噪声**：每进程取自身历史最小增量的 2 倍作为底噪扣除（自适应，各进程底噪不同）
6. **扣落盘尖峰**：同步监测 rollout/日志/WAL 文件大小增量，调用完成瞬间的写盘字节不计入输出
7. **字节 → token**：除以**自校准系数**。每次调用完成后，用它的真实 `output_tokens` ÷ 该调用流式区间实测字节得到一次校准样本，取最近 5 次的中位数作为当前系数（实测约 1600 B/token，随内容类型自动漂移修正）。**输出 <300 token 的调用不入样本**——小调用的 UI 固定帧开销会把系数抬高数倍（实测样本动辄 4000~8000 B/token，把读数压到真值的 1/4~1/5）
...
**精度说明**：字节 → token 的换算是统计近似的（受 UI 帧内容影响，单次调用内读数会围绕真值波动，长期积分约 ±5%），但字节速率本身是逐拍的真实观测。精确 token 计数仍由每次调用完成时的落盘记录提供，用于今日总量与均值——两者互为补充。
...
**调试日志**：面板运行时持续把三类数据追加到 `~/.zcode/speed-panel-debug.jsonl`（8MB 自动轮转）：`tick`（实时显示值/生效系数/统计图尾桶，活跃期逐拍+待机心跳 30s 一条）、`call`（每轮调用完成后的真实 token 与真实速度）、`cal`（每次校准样本与被污染排除标记）。离线验证工具同样记录这三类事件：
